                    ...
                    <div class="p-6 text-gray-900">
                        ...
                        <form action="{{route('webhook.update',[$webhookBucket->id, $webhook])}}" method="post">
                            ...
                            <div class="form-group">
                                ...
                                <select name="authentication_type" id="" class="form-control">
                                    ...
                                    @foreach ($authenticationTypes as $key => $type)
                                        <option value="{{ $type }}" {{$webhook->authentication_type == $type ? 'selected' :''}}>{{ $type }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select name="response_code" id="" class="form-control">
                                    ...
                                    @foreach ($statusCodes as $key => $code)
                                        <option value="{{ $code }}" {{$webhook->response_code == $code ? 'selected' :''}}>{{ $code }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="response_content" class="form-control">{{$webhook->response_content ?? " "}}</textarea>
